...
class SubGymMarketsMmRiskAvEnv_v1(AbidesGymMarketsEnv):
    """
   Market Maker risk averse v1 environment. It defines one of the ABIDES-Gym-markets environments.
   ...
   NB: The agent addresses risk aversion by defining its reward function using the Constant Absolute Risk Aversion (CARA) utility function.
...
Execution MM:
    ...
    def __init__(
        ...
        self.old_holdings: int = 0
        num_ns_day = (16 - 9.5) * 60 * 60 * 1e9
        step_length = self.timestep_duration
        self.num_steps_per_episode = num_ns_day / step_length
        self.remaining_time = 1 #normalized to 1
    ...
    def raw_state_to_state(self, raw_state: Dict[str, Any]) -> np.ndarray:
        ...
        Returns:
            - state: state representation defining the MDP for the market maker basic-risk averse v1 environment
        """
        # 0)  Preliminary
    ...
    def raw_state_to_reward(self, raw_state: Dict[str, Any]) -> float:
        ...
        Returns:mkt
            - reward: immediate reward computed at each step  for the market maker basic-risk averse v1 environment
        """
        if self.reward_mode == "dense":
            ...
            # 5) Reward 
            reward = self.a*(Execution_PnL) + np.exp(self.b*self.remaining_time)*np.sign(np.abs(holdings)- np.abs(self.old_holdings))
            self.remaining_time = self.remaining_time - 1/self.num_steps_per_episode
            self.old_holdings = holdings
            return reward
    ...
    def raw_state_to_update_reward(self, raw_state: Dict[str, Any]) -> float:
        ...
        Returns:
            - reward: update reward computed at the end of the episode for the market maker basic-risk averse v1 environment
        """
        if self.reward_mode == "dense":
            return 0
            ...
            # 5) Reward
            Inv_liquidation_gain = 0
            remaining_qt = holdings
            if np.sign(holdings)>0:
                #sell/bid side:
                for bid in bids:
                    if bid[1]<remaining_qt:
                       Inv_liquidation_gain += bid[0]*bid[1]
                       remaining_qt -= bid[1]
                    else:
                        Inv_liquidation_gain += bid[0]*remaining_qt
                        remaining_qt = 0
                    if remaining_qt == 0:
                        break

            elif np.sign(holdings)<0:
                #buy
                for ask in asks:
                    if ask[1]<remaining_qt:
                       Inv_liquidation_gain += ask[0]*ask[1]
                       remaining_qt -= ask[1]
                    else:
                        Inv_liquidation_gain += ask[0]*remaining_qt
                        remaining_qt = 0
                    if remaining_qt == 0:
                        break
                

            reward = self.alpha - np.exp(-self.risk_av_coeff*(Execution_PnL-Inv_liquidation_gain))
            return reward
    ...
    def raw_state_to_done(self, raw_state: Dict[str, Any]) -> bool:
        ...
        Returns:
            - done: flag that describes if the episode is terminated or not  for the market maker basic-risk averse v1 environment
        """
    ...
    def raw_state_to_info(self, raw_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            - reward: info dictionary computed at each step for the market maker basic-risk averse v1 environment
        """
