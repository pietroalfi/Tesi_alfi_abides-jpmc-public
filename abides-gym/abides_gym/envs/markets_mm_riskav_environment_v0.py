...
class SubGymMarketsMmRiskAvEnv_v0(AbidesGymMarketsEnv):
    """
   Market Maker risk averse v0 environment. It defines one of the ABIDES-Gym-markets environments.
   ...
   reward at the end of the day (i.e., Execution PnL and Inventory PnL + Risk Aversion component).

   NB: The agent accounts for risk aversion by providing seven possible penalty components in the reward function, all of which are Inventory-based.
...
Execution MM:
    ...
    def __init__(
        ...
        risk_av_coeff: float = 0.01, # in [0,1]
        pnl_inv_history_length: int = 10,
    ) -> None:
        ...
        self.risk_av_coeff = risk_av_coeff
        self.pnl_inv_history_length = pnl_inv_history_length
        ...
        self.inv_pnl = [] # inventroy pnl history for coeff calculation
    ...
    def raw_state_to_reward(self, raw_state: Dict[str, Any]) -> float:
        ...
        if self.reward_mode == "dense":
            ...
            # 3) Inventory PnL (DA VALUATRE SE TENERE DA CONTO IL CASH )
            Inventory_PnL =  holdings * np.diff(self.mid_price_hist)[0] 
            self.inv_pnl.append(Inventory_PnL)
           ...
           # 5.1) Reward with PnL vol
            if len(self.inv_pnl) < self.pnl_inv_history_length:
                reward = Execution_PnL + Inventory_PnL #+cash
            else:
                inv_pnl_vol = np.std(self.inv_pnl[-self.pnl_inv_history_length:])
                print("inv_pnl_vol:", inv_pnl_vol)
                print("inv_pnl_vol Reward component:", self.risk_av_coeff*inv_pnl_vol)                
                reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*inv_pnl_vol 
            """
            # 5.2) Reward with quadratic penalty
            reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*(Inventory_PnL**2)
            
            # 5.3) Reward with logarithmic penalty
            reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*np.log(1+np.abs(Inventory_PnL))

            # 5.4) Reward with Asymmetrically Dampened Negative PnL
            reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*(np.min([0,Inventory_PnL]))

            # 5.5) Reward with Asymmetrically Dampened Positive PnL
            reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*(np.max([0,Inventory_PnL]))
  
            # 5.6) Reward with Symmetrically Dampened Positive PnL (less intuitive)
            reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*Inventory_PnL

            # 5.7) Reward with Symmetrically Dampened Positive PnL
            reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*np.abs(Inventory_PnL)
            """
    ...
    def raw_state_to_update_reward(self, raw_state: Dict[str, Any]) -> float:
        ...
        if self.reward_mode == "dense":
            ...
            # 5.3) Reward with absolte value penalty
            reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*np.abs(Inventory_PnL)
            
            # 5.4) Reward with logarithmic penalty
            reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*np.log(1+np.abs(Inventory_PnL))

            # 5.5) Reward with Asymmetrically Dampened Negative PnL
            reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*(np.min([0,Inventory_PnL]))

            # 5.6) Reward with Asymmetrically Dampened Positive PnL
            reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*(np.max([0,Inventory_PnL]))
  
            # 5.7) Reward with Symmetrically Dampened Positive PnL (less intuitive)
            reward = Execution_PnL + Inventory_PnL - self.risk_av_coeff*Inventory_PnL
            """
            # 6) Order Size Normalization of Reward
            reward = reward / (2*self.order_fixed_size)
